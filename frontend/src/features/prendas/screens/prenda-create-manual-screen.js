...
import { create_prenda_manual, fetch_prendas_for_user } from '../state/prendas-slice';
...
export function PrendaCreateManualScreen() {
  ...
  const handle_submit = async () => {
    ...
    try {
      ...
      set_local_error(error?.message ?? 'No se pudo guardar la prenda');
    } finally {
      set_is_processing_submission(false);
    }
  };
      ...
      {Boolean(visible_error) && (
        ...
        >
          <Text selectable style={prenda_create_manual_screen_styles.submit_button_text}>
            {is_submitting ? 'Guardando...' : 'Guardar prenda'}
          </Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoidingView>

...
class PrendaCRUD:
	...
	def get_all(db: Session) -> list[Prenda]:
		return db.query(Prenda).options(selectinload(Prenda.colores)).all()
	...
	def get_by_id(db: Session, prenda_id: int) -> Prenda | None:
		return (
			db.query(Prenda)
			.options(selectinload(Prenda.colores))
			.filter(Prenda.id == prenda_id)
			.first()
		)
	...
	def delete(db: Session, prenda_id: int) -> bool:
		...
		db.delete(prenda)
		PrendaCRUD._eliminar_outfits_sin_prendas(db)
		try:
			db.commit()
		except Exception:
			db.rollback()
